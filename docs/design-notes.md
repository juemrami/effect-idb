...
# a fundamental IDB constraint that no Effect abstraction can fully hide...
 - IDB transactions auto-commit when control returns to the event loop with no pending requests.
 - A fetch (or any async work) in the middle of a transaction is a guaranteed transaction death!
...
the 3 ways to solve this are:
	...
	2. try to make an easier path
	 - The number 1 way to guide a consumer would be to not allow for Transaction layers to be blanket provided at comp time with a typescript error.
...
**read from bottom up**
	```ts
	const data = yield* fetch("https://...")
	yield* Effect.gen(function*() { // <-  4. program ran
		const objectStoreA = yield* ObjectStoreA
		const objectStoreB = yield* ObjectStoreB
