...
const createServiceFromDB = (db: IDBDatabase) => {
    const createObjectStore = (
        name: string, options?: IDBObjectStoreParameters, indexes: IDBObjectStoreIndexParams[] = []
    ) => pipe(
        ...
        Effect.tap(store => Effect.forEach(indexes,
            (index) => Effect.try({
    ...
    const deleteObjectStore = (name: string) => {
        return pipe(Effect.try({
            ...
            catch: (error) => new IDBDatabaseObjectStoreDeletionError({
    ...
    const use = <A, E, R>(cb: (txn: IDBDatabase) => Effect.Effect<A, E, R>) => Effect.gen(function* () {
        return yield* cb(db);
    })
    return {
        ...
        name: db.name,
        version: db.version,
        objectStoreNames: Effect.sync(() => Array.from(db.objectStoreNames) as string[]),
        createObjectStore,
        deleteObjectStore,
        use,
    }
...
export class IDBDatabaseService extends Context.Tag("IDBDatabaseService")<IDBDatabaseService, DBServiceShape>() {
    static make = (config: IDBDatabaseConfig) => Layer.scoped(IDBDatabaseService,
        Effect.gen(function* () {
    ...
    static makeLive = (config: IDBDatabaseConfig) => Layer.provide(
        IDBDatabaseService.make(config),
        IDBFactoryService.Live
    );
    static makeTest = (config: IDBDatabaseConfig) => Layer.provide(
        IDBDatabaseService.make(config),
...
>() {
    static DefaultNoDependencies = Layer.effect(IDBFactoryService, Effect.gen(function* () {
        const indexedDB = yield* IDBFactoryImplementation;
        return {
            open: (config: IDBDatabaseConfig) => Effect.gen(function* () {
                ...
                const dbConnection = yield* Effect.async<IDBDatabase, IDBDatabaseOpenError>((resume, _signal) => {
                    ...
                    let upgradeFiber: Fiber.RuntimeFiber<void, any> | null = null;
                    ...
                    request.onerror = () => {
                        ...
                        if (isKnownDOMException(request.error, IDBRequestExceptionType)) {
                            resume(
                                Effect.fail(new IDBDatabaseOpenError({
